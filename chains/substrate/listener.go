...
func (l *listener) pollBlocks() error {
	...
	for {
		...
		default:
			...
			if err != nil {
				l.log.Error("Failed to process events in block", "block", currentBlock, "err1", err)
				retry--
				continue
			}
...
func (l *listener) processEvents(hash types.Hash) error {
	...
	l.log.Trace("Fetching block for events -> DecodeEventRecords", "err", err)

	if err != nil {
		l.log.Error("Failed to process events in block", "block", hash.Hex(), "err2", err)
		// return err
	}
...
func (l *listener) handleEvents(evts utils.Events) {
	if l.subscriptions[FungibleTransfer] != nil {
		l.log.Trace("Enter Handling FungibleTransfer event", evts.ChainBridge_FungibleTransfer)
