...
func (w *writer) createErc20Proposal(m msg.Message) bool {
	...
	w.log.Info("++++++++++++Creating erc20 proposal", "src", m.Source, "nonce", m.Nonce, "data", data, "dataHash", dataHash)

	w.log.Info("++++++++++++Creating erc20 proposal", "src", m.Source, "nonce", m.Nonce, "shouldVote", w.shouldVote(m, dataHash))
...
func (w *writer) watchThenExecute(m msg.Message, data []byte, dataHash [32]byte, latestBlock *big.Int) {
	...
	for i := 0; i < ExecuteBlockWatchLimit; i++ {
		...
		default:
			...
			w.log.Info("=================Watching for finalization event", "src", m.Source, "debug point", `1`)
			w.log.Info("=================Watching for finalization event", "src", m.Source, "debug point", `1`, "proposal event", utils.ProposalEvent)

			// query for logs
			query := buildQuery(w.cfg.bridgeContract, utils.ProposalEvent, latestBlock, latestBlock)
			w.log.Info("=================Watching for finalization event", "QUERY", query)

			for waitRetrys := 0; waitRetrys < 10; waitRetrys++ {
				evts1, err1 := w.conn.Client().FilterLogs(context.Background(), query)
				if err1 != nil {
					w.log.Error("Failed to fetch logs", "err", err1)
					return
				}

				w.log.Info("=================Watching for finalization event", "src", m.Source, "debug point", `1`, "loop", evts1)
			...
			w.log.Info("=================Watching for finalization event", "src", m.Source, "debug point", `2`)
			w.log.Info("=================Watching for finalization event", "src", m.Source, "debug point", `3`, "evts", evts)
			...
			for _, evt := range evts {
				...
				w.log.Info("=================Watching for finalization event", "src", m.Source, "debug point", `4`, "Source", m.Source == msg.ChainId(sourceId))
				w.log.Info("=================Watching for finalization event", "src", m.Source, "debug point", `5`, "Nonce", m.Nonce.Big().Uint64() == depositNonce)
				w.log.Info("=================Watching for finalization event", "src", m.Source, "debug point", `6`, "IsFinalized", utils.IsFinalized(uint8(status)))

				if m.Source == msg.ChainId(sourceId) &&
					m.Nonce.Big().Uint64() == depositNonce &&
					utils.IsFinalized(uint8(status)) {
					w.log.Info("=================Watching for finalization event", "src", m.Source, "debug point", `7`)
	...
	for i := 0; i < TxRetryLimit; i++ {
		...
		default:
			...
			if err == nil {
				...
				if w.metrics != nil {
					w.log.Info("Submitted proposal vote", "tx", 1)
